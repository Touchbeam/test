...
// COLLECTION 대신 SEASON 으로 대체 합니다
...
const handleLikeClick = ( productId, categoryId ) => {
  ...
  useEffect(()=>{
      setFetching(true)
      getListFilter({page, size},"SEASON", null, null, null, null, sortBy, sortOrder).then(data => {
          console.log(data)
          setServerData(data) 
          setFetching(false)
      })
  ...
  return (
    <>
      ...
      <Container>
        ...
        <Row>
        {serverData.dtoList.filter(product => product.productBrand == "UGG").map((product,index) => (
            ...
            <Card className='mb-5 '>
              <div className='image-wrapper mx-auto my-3' onClick={() => moveToRead(product.productId)}>
                <Card.Img variant="top " style={{ width: '100%' , height:'100%'}} 
                          src={`${host}/product/view/${product.uploadFileNames[0]}`}  onError={(e) => e.target.src = defaultImage} />
